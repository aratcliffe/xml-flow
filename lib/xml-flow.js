...
const flow = function xmlFlow(inStream, options = defaults) {
  ...
  saxStream.on('opentag', node => {
    ...
    topNode = {
      $name: stripPrefix ? stripPrefixFn(node.name) : node.name,
      $attrs: stripPrefix ? Object.keys(node.attributes).reduce((o, name) => {o[stripPrefixFn(name)] = node.attributes[name]; return o;}, {}) : node.attributes;
    };
